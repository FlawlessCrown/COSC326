...
public class Arithmetic {
    ...
    public static void normal(int current_index, int current_number, int number) {
        if (end) {
            System.out.println(current_index);
            return;
        }
        if (current_index == numbers_array.size()) {
        ...
        next_value = numbers_array.get(current_index);
        if (number < target && current_number < target) {
            current_operation += "+";
            normal((current_index + 1), (current_number + number), next_value);
            current_operation = current_operation.substring(0, (current_operation.length() - 1));
        }
        if (end) {return;}
        current_operation += "*";
        if (number == 0) {
            normal((current_index + 1), (current_number * next_value), number);
        } else {
            normal((current_index + 1), current_number, (number * next_value));
        }
        current_operation = current_operation.substring(0, current_operation.length()-1);
    ...
    public static void left_to_right(int current_index, int current_number) {
        if (end) {
            return;
        }
        if (current_index == numbers_array.size()) {
        ...
        addition_num = current_number + numbers_array.get(current_index);
        multiplication_num = current_number * numbers_array.get(current_index);
        if (addition_num <= target) {
            current_operation += "+";
            left_to_right((current_index + 1), addition_num);
            current_operation = current_operation.substring(0, current_operation.length()-1);
        }
        if (end) {
            return;
        }
        if (multiplication_num <= target) {
            current_operation += "*";
            left_to_right((current_index + 1), multiplication_num);
            current_operation = current_operation.substring(0, current_operation.length()-1);
        }
    }
